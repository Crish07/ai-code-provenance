...
func TestVerifier_AITrackedLinesFullyCovered(t *testing.T) {
	...
	v := Verifier{Git: git.Reader{Root: root}, Store: store}
	res, err := v.Verify(context.Background(), Request{Scope: git.ScopeStaged})
	if err != nil {
	...
	if res.Coverage != 1 || res.Status != StatusOK {
	...
	if fr := singleFileReport(t, res.Files, "a.go"); fr != nil {
		...
		lr := fr.AddedLines[0]
		if lr.Content != "beta" || lr.Source != LineSourceAI || lr.SessionID != start.SessionID {
...
func TestVerifier_HumanAddedLinesAreUncovered(t *testing.T) {
	...
	strict, err := v.Verify(context.Background(), Request{Scope: git.ScopeStaged, Strict: true})
	if err != nil {
		t.Fatalf("Verify strict: %v", err)
	}
	if strict.Status != StatusFailed {
	...
	if fr := singleFileReport(t, res.Files, "a.go"); fr != nil {
		...
		if fr.AddedLines[0].Content != "beta" || fr.AddedLines[0].Source != LineSourceAI {
			t.Fatalf("line 0=%#v want beta/AI", fr.AddedLines[0])
		}
		if fr.AddedLines[1].Content != "gamma" || fr.AddedLines[1].Source != LineSourceUnknown || fr.AddedLines[1].SessionID != "" {
...
func TestVerifier_EmptyDiffIsOK(t *testing.T) {
	...
	v := Verifier{Git: git.Reader{Root: root}, Store: store}
	res, err := v.Verify(context.Background(), Request{Scope: git.ScopeStaged, Strict: true})
	if err != nil {
		t.Fatalf("Verify: %v", err)
	}
	if res.TotalAddedLines != 0 || res.Coverage != 1 || res.Status != StatusOK {
...
func TestVerifier_DuplicateLinesRespectProvenanceCount(t *testing.T) {
	...
	if fr := singleFileReport(t, res.Files, "a.go"); fr != nil {
		...
		if fr.AddedLines[0].Source != LineSourceAI || fr.AddedLines[1].Source != LineSourceUnknown {
...
func TestVerifier_WorktreeScopeMatchesStaged(t *testing.T) {
	...
	v := Verifier{Git: git.Reader{Root: root}, Store: store}
	res, err := v.Verify(context.Background(), Request{Scope: git.ScopeWorktree})
	if err != nil {
		t.Fatalf("Verify: %v", err)
	}
...
func singleFileReport(t *testing.T, files []FileReport, path string) *FileReport {
	t.Helper()
	if len(files) == 0 {
		return nil
	}
